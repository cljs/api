...
### DESCRIPTION
###   Given a directory name, yaml-prettify all its *.md files in place, assuming it is just Jekyll front matter.

# PRETTIFIER FROM:
#   https://github.com/rkj/ya2yaml/blob/master/lib/ya2yaml.rb
...
class Ya2YAML
	...
	def _ya2yaml(obj)
    # REMOVED: caused an error when using Ruby 2.0
...
def prettify_yaml_file!(filename)
  content = File.read(filename)
  begin
    new_content = YAML.load(content).ya2yaml(:syck_compatible => true)
    File.write(filename, new_content + "---")
  rescue
    puts "FAILED for", filename
  end
end

prettify_yaml_file! ARGV[0]
