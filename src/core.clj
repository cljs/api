...
(defn get-versions-since
  [latest]
  (let [versions (get-repo-versions)]
    (if-not latest
      versions
      (let [get-num #(Integer/parseInt (subs % 1))
            latest-num (get-num latest)]
        (filter #(> (get-num %) latest-num) versions)))))
...
(defn write-history!
  [version-map latest]
  (spit history-filename
        (join "\n" (concat [latest] (make-history-lines version-map)))))
...
(defn -main
  ...
  (let [[latest history] (get-symbol-history)
    ...
    (doseq [version versions]
      ...
      (binding [*cljsdoc-dir* (str "docs/" version)
